    ...
    <div className={styles.trade}>
      ...
      {tokenInfo && (
        <div className="glass-panel" style={{ padding: '16px', marginTop: '16px' }}>
          <h2>{tokenInfo.symbol} - {tokenInfo.name}</h2>
          <p>Price: ${tokenInfo.price}</p>
          <p>24h Change: {tokenInfo.price_change_24h}%</p>
          <div style={{ marginTop: '12px', display: 'flex', gap: '12px' }}>
            <button className="glass-panel" onClick={() => handleTrade('buy')}>Buy</button>
            <button className="glass-panel" onClick={() => handleTrade('sell')}>Sell</button>
          </div>
        </div>
      )}
    </div>
